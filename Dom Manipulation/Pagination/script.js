...
//get div where the table need to be created
const list_element = document.getElementById('list');

//get div where the pagination buttons need to be created
const pagination_element = document.getElementById('buttons');
...
//function to create table and append the list items to the table
...
table.setAttribute('class', 'table table-striped table-bordered mb-0');
table.setAttribute('id','table')

//create the table head(thead) and append it to the table
...
//create the table body(tbody) and append it to the table
...
//add the list items to table body
...
SetupPaginationButton(list_items, pagination_element, rows);
}

//function to create pagination buttons
function SetupPaginationButton (items, wrapper, rows_per_page) {

let page_count = Math.ceil(items.length / rows_per_page);
wrapper.innerHTML = ``;

var maxLeft = current_page - Math.floor(cols / 2);
var maxRight = current_page + Math.floor(cols / 2);
if (maxLeft < 1) {
  maxLeft = 1;
  maxRight = cols;
}
...
//for first button, it should be displayed only when first(1) is not the currentpage 
if (current_page !== 1) {
  let first=document.createElement('button');
  first.innerText='« First';
  first.setAttribute('class', 'btn btn-sm btn-warning px-3 mx-2');
  first.addEventListener('click', ()=> handlePageClick(items,1));
  wrapper.appendChild(first);
  }

//for previous button, to diplay the previous page from the currentpage 
let previous=document.createElement('button');
previous.innerText='‹ Previous';
previous.setAttribute('class', 'btn btn-sm btn-dark px-3 mx-2');
previous.addEventListener('click',()=> handleMoveClick(items,-1))
wrapper.appendChild(previous);

for (let i = maxLeft; i <= maxRight; i++) {	
  let button = document.createElement('button');
  button.innerText = i;
  button.setAttribute('class', 'btn btn-sm btn-info mx-2 px-3');
  button.addEventListener('click', ()=> handlePageClick(items,i));
  wrapper.appendChild(button);
}

//for next button, to display the nextpage the currentpage 
let next=document.createElement('button');
next.innerText='Next ›';
next.setAttribute('class', 'btn btn-sm btn-dark px-3 mx-2');
next.setAttribute
next.addEventListener('click',()=> handleMoveClick(items,1));
wrapper.appendChild(next);

//for last button, it should be displayed only when currentpage is not the last (20rth) page
if (current_page !== page_count) {
  let last=document.createElement('button');
  last.innerText='Last »';
  last.setAttribute('class','btn btn-sm btn-warning');
  last.addEventListener('click', ()=> handlePageClick(items,page_count));
  wrapper.appendChild(last);
}
}

//function to handle button clicks of page number, first and last
function handlePageClick(items,page) {
current_page = page;
DisplayList(items, list_element, rows, current_page);
};

//function to handle button click of previous and next
function handleMoveClick(items,page){
current_page+=page;
if (current_page > 20) current_page = 1;
  else if (current_page < 1) current_page= 20;
DisplayList(items, list_element, rows, current_page);
}
